...
<br>
<br>
I did not find anything there, so my next move was to check `robots.txt`. This is a simple text file located in the website's root directory that tells search engine crawlers which pages or sections of the site they are allowed or not allowed to visit.
<br>
<br>
...
<br>
<br>
Again, I checked the HTML source page, but found none. The wording on this page "_but there's more to discover..._" suggests that there's more, usually other directories on the site that's hidden from the user.
<br>
<br>
I actually ran `nmap` scan first, but it didn't work on my machine. Since we don't need to perform a network scan, `gobuster` is our next best step.
<br>
<br>
<img width="1428" height="1462" alt="05" src="https://github.com/user-attachments/assets/c9e07188-4413-4ed4-ab0d-974b576ce3a3" />
<br>
<br>
For this, we can use `gobuster` (or dirbuster, whichever you fancy). It doesn't actually know what to look for, instead it uses a wordlist that appends words at the end of the target URL to brute-force the server for hidden directories, and logs the links that return a valid POST request.
<br>
<br>
Command used:
```
...
`-u http://MACHINE_IP:5000/cupids_secret_vault` is the target URL.\
